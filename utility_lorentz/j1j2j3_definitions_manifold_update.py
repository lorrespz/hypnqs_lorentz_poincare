...
class LorentzGRU(nn.Module):
    ...
    def __init__(self, input_dim, num_units, spatial_clamp, k=1.0):
        ...
        for gate in ['z', 'r', 'h']:
            # W gates map Hidden -> Hidden (50 -> 50)
            # Shapes here stay (num_units, num_units)
    ...
    def forward(self, inputs, state):
        ...
        #---------------------------------------------------------------------------------
        # Option 2: Tangent-space update (see j1j2j3_definitions_tangent_update.py)
        #--------------------------------------------------------------------------------
        #--------------------------------------------------------------------------------
    ...
    def _apply_spatial_clamp(self, x, eps=1e-5):
        ...
        # Constrain the norm BEFORE it hits the exponential functions.
        max_norm = (self.spatial_clamp - eps) / (self.k**0.5)
