...
# This code defines the LorentzRNN and LorentzGRU class (for J1J2J3 model)
# The LorentzGRU class in this file uses the tangent space update method
...
class LorentzRNN(nn.Module):
    def __init__(self, input_dim, num_units, spatial_clamp, k=1.0):
            super().__init__()
    ...
    def forward(self, inputs, state):
        ...
        #only for j1j2j3 model to avoid the state from being pushed to the edge of the hyperboloid
...
class LorentzGRU(nn.Module):
    ...
    def forward(self, inputs, state):
        ...
        #--------------------------------------------------------------------------------
        # Option 1: Manifold update (see j1j2j3_definitions_manifold_update.py)
        #--------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------
        #Option 2: Tangent-space update
        #--------------------------------------------------------------------------------
        # a. Map both state and h_tilde to the tangent space at the origin
        u_state = hc_math.logmap0(state,k=self.k_tensor, is_tan_normalize=False)
        u_tilde = hc_math.logmap0(h_tilde,k=self.k_tensor, is_tan_normalize=False)
        # b. Subtract in Euclidean space (This is always stable!)
        u_diff = u_tilde - u_state
        # c. Scale by the gate and map back
        update_step = project_lorentz_manual(hc_math.expmap0(z * u_diff,k=self.k_tensor))
    ...
    def _apply_spatial_clamp(self, x, eps=1e-5):
        # More aggressive scaling than 'projection_lorentz_manual'
        # Force the hidden state to stay within a strict radius
        max_norm = (self.spatial_clamp - eps) / (self.k**0.5)
